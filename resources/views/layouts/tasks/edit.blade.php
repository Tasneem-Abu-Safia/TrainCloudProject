            ...
            <div class="card-body">
                <form method="POST" action="{{ route('tasks.update', $task->id) }}" enctype="multipart/form-data">
                            ...
                            <select class="form-control {{ $errors->has('course') ? 'is-invalid' : '' }}"
                                ...
                                @foreach($courses as $course)
                                    <option value="{{ $course->id }}"
                                            @if($course->id == $task->course_id) selected @endif>{{ $course->name }}</option>
                                @endforeach
                            </select>
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-lg-6">
                            ...
                            <input type="date" class="form-control {{ $errors->has('end_date') ? 'is-invalid' : '' }}"
                                   name="end_date" id="end_date" value="{{ old('end_date', $task->end_date) }}" required
                                   min="{{ date('Y-m-d') }}">
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-lg-6">
                            ...
                            <textarea class="form-control" name="description" rows="2" required
                                      placeholder="Type a Description">{{ old('description', $task->description) }}</textarea>
                        </div>
                        <div class="col-lg-6">
                            <label for="mark">Mark</label>
                            <input type="number" class="form-control {{ $errors->has('mark') ? 'is-invalid' : '' }}"
                                   name="mark" id="mark" min="0" value="{{ old('mark', $task->mark) }}"
                    ...
                    <div class="form-group">
                        ...
                        @if ($task->file)
                            <a href="{{ $task->file }}" class="btn btn-light-primary" target="_blank">Open
                                File</a>
                        @else
